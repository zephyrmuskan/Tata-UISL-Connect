...
export const CustomerDashboard: React.FC = () => {
  ...
  const getStatusColor = (status: Application['currentStatus']) => {
    switch (status) {
      ...
      case 'Document Verification': return 'bg-yellow-50 text-yellow-700 border-yellow-100 dark:bg-yellow-950/20 dark:text-yellow-400';
      case 'Under Review': return 'bg-purple-50 text-purple-700 border-purple-100 dark:bg-purple-950/20 dark:text-purple-400';
      ...
      case 'Connection Completed': return 'bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400';
  ...
  const activeApplications = applications.filter(a => a.currentStatus !== 'Connection Completed' && a.currentStatus !== 'Rejected');
  const completedConnections = applications.filter(a => a.currentStatus === 'Connection Completed');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Total Applications</span>
            <span className="block text-2xl font-extrabold text-gray-900 dark:text-white mt-1">{applications.length}</span>
          </div>
          <div className="h-10 w-10 bg-blue-50 dark:bg-slate-700 rounded-xl flex items-center justify-center text-tata-blue dark:text-tata-blue-light">
            <FileText size={20} />
          </div>
        </div>
        ...
        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Pending Status</span>
            <span className="block text-2xl font-extrabold text-gray-900 dark:text-white mt-1">{activeApplications.length}</span>
          </div>
          <div className="h-10 w-10 bg-yellow-50 dark:bg-slate-700 rounded-xl flex items-center justify-center text-yellow-600 dark:text-yellow-400">
            <Clock size={20} />
          </div>
        </div>
        ...
        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Active Connections</span>
            <span className="block text-2xl font-extrabold text-gray-900 dark:text-white mt-1">{completedConnections.length}</span>
          </div>
          <div className="h-10 w-10 bg-green-50 dark:bg-slate-700 rounded-xl flex items-center justify-center text-green-600 dark:text-green-400">
            <CheckCircle2 size={20} />
          </div>
        </div>

        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Verification Complete</span>
